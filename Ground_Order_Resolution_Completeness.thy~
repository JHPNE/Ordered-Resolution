  ...
  next
    ...
    proof (rule notI)
      ...
      moreover have "\<forall>L \<in># D'. L \<prec>\<^sub>l Pos A\<^sub>C"
        ...
        using calculation literal.order.order.strict_trans by blast
...
proof -
  ...
  from \<open>D \<in> N\<close> \<open>C \<prec>\<^sub>c D\<close> have "A \<in> rewrite_sys N D"
    using C_prod epsilon_subset_if_less_cls by auto
    ...
    proof (cases L)
      ...
      proof -
        ...
        proof -
          ...
          also have "\<dots> = (\<Union>D' \<in> {D' \<in> {y \<in> N. y \<prec>\<^sub>c C} \<union> {C} \<union> {y \<in> N. C \<prec>\<^sub>c y}. D' \<prec>\<^sub>c D}. epsilon N D')"
            using C_in clause.order.antisym_conv3 by auto
          also have "\<dots> = (\<Union>D' \<in> {y \<in> N. y \<prec>\<^sub>c C \<and> y \<prec>\<^sub>c D} \<union> {C} \<union> {y \<in> N. C \<prec>\<^sub>c y \<and> y \<prec>\<^sub>c D}. epsilon N D')"
          ...
          also have "\<dots> = (\<Union>D' \<in> {y \<in> N. y \<prec>\<^sub>c C} \<union> {C} \<union> {y \<in> N. C \<prec>\<^sub>c y \<and> y \<prec>\<^sub>c D}. epsilon N D')"
            by (metis (lifting) assms(3) clause.order.dual_order.strict_trans)
          also have "\<dots> = rewrite_sys N C \<union> epsilon N C \<union> (\<Union>D' \<in> {y \<in> N. C \<prec>\<^sub>c y \<and> y \<prec>\<^sub>c D}. epsilon N D')"
            by (auto simp: rewrite_sys_def)
          finally show ?thesis
            using C_prod
...
proof -
  ...
  have "rewrite_sys N D \<subseteq> rewrite_sys N C"
    using D_lt_C rewrite_sys_subset_if_less_cls by auto
  ...
  proof (elim disjE conjE)
    ...
    thus "rewrite_sys N C \<TTurnstile> D"
      using L_in \<open>rewrite_sys N D \<subseteq> rewrite_sys N C\<close> by auto
  next
    assume "L = Neg A" and "A \<notin> rewrite_sys N D"
...
proof -
  ...
  next
    show "is_maximal (Pos A) D"
      using \<open>is_strictly_maximal (Pos A) D\<close> by auto
  qed simp_all
  hence "\<not> (\<prec>\<^sub>c)\<^sup>=\<^sup>= C D"
    using clause.order.not_less by blast
  thus ?thesis
  proof (rule contrapos_np)
    from D_prod show  "A \<notin> rewrite_sys N C \<Longrightarrow> (\<prec>\<^sub>c)\<^sup>=\<^sup>= C D"
      using \<open>D \<prec>\<^sub>c C\<close> epsilon_subset_if_less_cls by blast
  qed
qed
...
proof (induction C arbitrary: D rule: wfp_induct_rule)
  ...
  next
    ...
    next
      ...
      proof (cases "entails (rewrite_sys N C) C'")
        case True
        then show ?thesis
          using C_def entails_def by force
      next
        case False
        show ?thesis
        proof (cases "is_strictly_maximal (Pos A) C")
          ...
          then show ?thesis
            ...
            using C_def entails_def by blast
        next
          case False
          ...
          ultimately have "entails (rewrite_sys N C) (add_mset (Pos A) C')"
            ...
            by (simp add: I_def true_clss_def)
          then show ?thesis
            using C_def entails_def by fastforce
        qed
      qed
    qed
  qed
  ...
  proof (elim disjE exE)
    ...
    hence "entails (rewrite_sys N C) C"
      by (simp only: i)
    thus ?thesis
      using lift_entailment_to_Union[OF \<open>C \<in> N\<close>] entails_def by argo
  next
    fix A
    ...
    moreover from eps have "A \<in> \<Union> (epsilon N `N)"
      using \<open>C \<in> N\<close> epsilon_subset_Union_epsilon
      by fast
  ...
  proof (elim disjE exE)
    ...
    hence "entails (rewrite_sys N C) C"
      unfolding i by simp
    thus ?thesis
      using lift_interp_entails[OF \<open>C \<in> N\<close> _ that] entails_def by argo
  next
...
lemma (in ground_order_resolution_calculus) model_construction:
  ...
  assumes "saturated N" and "{#} \<notin> N" and C_in: "C \<in> N"
  shows
    "entails (\<Union>D \<in> N. epsilon N D) C"
